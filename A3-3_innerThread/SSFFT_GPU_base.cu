...
SSFFT_GPU_Short_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nFFTs, int nSMs, int dummy){ // in-place
	...
	for (int N = 0; N < (nFFTs+parallel*blockDim.y-1)/(parallel*blockDim.y); N++) {

		if ((blockIdx.x % nSMs) == 0 && threadIdx.y + (blockIdx.x/nSMs)*blockDim.y + N*parallel*blockDim.y < nFFTs) {

		for (int i = 0; i < FFT_size / blockDim.x; i++) {
			s_input[blockDim.x*i + threadIdx.x + threadIdx.y*FFT_size] 
			= d_input[blockDim.x*i + threadIdx.x + threadIdx.y*FFT_size + ((blockIdx.x/nSMs)*FFT_size*blockDim.y) + N*FFT_size*blockDim.y*parallel];
		}
		__syncthreads();
		...
		for (int i = 0; i < FFT_size / blockDim.x; i++) {
			d_output[blockDim.x*i + threadIdx.x + threadIdx.y*FFT_size + ((blockIdx.x/nSMs)*FFT_size*blockDim.y) + N*FFT_size*blockDim.y*parallel] 
			= s_input[blockDim.x*i + threadIdx.x + threadIdx.y*FFT_size];
		}
		__syncthreads();
		}
...
SSFFT_GPU_Long_step1_base(float2 *d_input, float2 *d_output, int Na, int FFT_size, int Na_exp, int Nb_exp, int nFFTs, int nSMs){ // Na = nFFTs, Nb = FFT_size
	...
	for (int N = 0; N < (nFFTs+parallel-1)/parallel; N++) {

		if (blockIdx.x/nSMs + parallel*N < nFFTs)
		...
		for (int iter = 0; iter < (Na+(nSMs*blockDim.y-1))/(nSMs*blockDim.y); iter++) {
			...
			for (int i = 0; i < FFT_size / blockDim.x; i++) {
				...
				= d_input[(Na*(FFT_size/2))*i + Na*threadIdx.x + threadIdx.y + blockDim.y*(blockIdx.x%nSMs) + blockDim.y*nSMs*iter + Na*FFT_size*(blockIdx.x/nSMs) + N*Na*FFT_size*parallel];
			}
			__syncthreads();
			...
			for (int i = 0; i < FFT_size / blockDim.x; i++) {
				d_output[(Na*(FFT_size/2))*i + Na*threadIdx.x + threadIdx.y + blockDim.y*(blockIdx.x%nSMs) + blockDim.y*nSMs*iter + Na*FFT_size*(blockIdx.x/nSMs) + N*Na*FFT_size*parallel] 
				= s_input[threadIdx.x + blockDim.x*i + threadIdx.y*FFT_size];
			}
			__syncthreads();
			}
...
SSFFT_GPU_Long_step2_base(float2 *d_input, float2 *d_output, int FFT_size, int Nb, int Na_exp, int Nb_exp, int nFFTs, int nSMs){ // Na = nFFTs, Nb = FFT_size
	...
	for (int N = 0; N < (nFFTs+parallel-1)/parallel; N++) {
		...
		for (int iter = 0; iter < (Nb+(nSMs*blockDim.y-1))/(nSMs*blockDim.y); iter++) {
			...
			for (int i = 0; i < FFT_size / blockDim.x; i++) {
				s_input[threadIdx.x + blockDim.x*i + threadIdx.y*FFT_size] 
				= d_input[threadIdx.x + blockDim.x*i + threadIdx.y*FFT_size + blockDim.y*(blockIdx.x%nSMs)*FFT_size + blockDim.y*nSMs*FFT_size*iter + FFT_size*Nb*(blockIdx.x/nSMs) + N*FFT_size*Nb*parallel];
			}
			__syncthreads();
			...
			for (int i = 0; i < FFT_size / blockDim.x; i++) {
				d_output[Nb*threadIdx.x + (Nb*(FFT_size/2))*i + threadIdx.y + blockDim.y*(blockIdx.x%nSMs) + blockDim.y*nSMs*iter + FFT_size*Nb*(blockIdx.x/nSMs) +  N*FFT_size*Nb*parallel] 
				= s_input[blockDim.x*i + threadIdx.x + threadIdx.y*FFT_size];
			}
			__syncthreads();
			}
...
void SSFFT_GPU_Short_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nFFTs, int nSMs) {
	...
	if (nFFTs < (parallel*blockDim_y)) 
		gridSize = ((nFFTs%(parallel*blockDim_y)+blockDim_y-1)/blockDim_y)*nSMs;

	SSFFT_GPU_Short_base<<<gridSize, blockSize, 32768>>>(d_input, d_output, FFT_exp, FFT_size, nFFTs, nSMs, 0);
}
...
void SSFFT_GPU_Long_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nFFTs, int nSMs) {
	...
	SSFFT_GPU_Long_step1_base<<<gridSize, blockSize1, 32768>>>(d_input, d_output, Na, Nb, Na_exp, Nb_exp, nFFTs, nSMs); // Na_exp, Nb_exp
	SSFFT_GPU_Long_step2_base<<<gridSize, blockSize2, 32768>>>(d_output, d_input, Na, Nb, Na_exp, Nb_exp, nFFTs, nSMs); // Na_exp, Nb_exp
