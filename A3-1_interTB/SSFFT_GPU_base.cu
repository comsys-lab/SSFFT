...
SSFFT_GPU_Short_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nSMs){ // in-place
	...
	if (threadIdx.y == 0 && (blockIdx.x % nSMs) == 0) {

    for (int i = 0; i < FFT_size / blockDim.x; i++) {
        s_input[blockDim.x*i + threadIdx.x] = d_input[blockDim.x*i + threadIdx.x + ((blockIdx.x/nSMs)*FFT_size)];
    }
    __syncthreads();
    ...
    for (int i = 0; i < FFT_size / blockDim.x; i++) {
        d_output[blockDim.x*i + threadIdx.x + ((blockIdx.x/nSMs)*FFT_size)] = s_input[blockDim.x*i + threadIdx.x];
    }
    __syncthreads();
	}
...
void SSFFT_GPU_Short_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nFFTs, int nSMs) {
	...
	for (int i = 0; i < (nFFTs+parallel-1)/parallel; i++) {
		...
		SSFFT_GPU_Short_base<<<gridSize, blockSize, 32768>>>(d_input_iter, d_output_iter, FFT_exp, FFT_size, nSMs);
	}
}
