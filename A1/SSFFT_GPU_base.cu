...
void SSFFT_GPU_Short_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nFFTs, int nSMs) {
	...
	SSFFT_GPU_Short_base<<<nSMs, blockSize, 32768>>>(d_input, d_output, FFT_exp, FFT_size, nSMs);
}
...
void SSFFT_GPU_Long_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nFFTs, int nSMs) {
	...
	// SSFFT_GPU_Long_step1_base<<<nFFTs*gridSize[idx], blockSize_step1[idx], sharedSize_step1[idx]>>>(d_input, d_output, Na, Nb, Na_exp, Nb_exp, nFFTs); // Na_exp, Nb_exp
	// SSFFT_GPU_Long_step2_base<<<nFFTs*gridSize[idx], blockSize_step2[idx], sharedSize_step2[idx]>>>(d_output, d_input, Na, Nb, Na_exp, Nb_exp, nFFTs); // Na_exp, Nb_exp
	SSFFT_GPU_Long_step1_base<<<nSMs, blockSize1, 32768>>>(d_input, d_output, Na, Nb, Na_exp, Nb_exp, nFFTs, nSMs); // Na_exp, Nb_exp
	SSFFT_GPU_Long_step2_base<<<nSMs, blockSize2, 32768>>>(d_output, d_input, Na, Nb, Na_exp, Nb_exp, nFFTs, nSMs); // Na_exp, Nb_exp
}
...
void SSFFT_benchmark_base(float2 *d_input, float2 *d_output, int FFT_exp, int FFT_size, int nFFTs, int nSMs, double *FFT_time){
	...
	for (int i = 0; i < nFFTs; i++) {
		d_input_iter = d_input+(i*FFT_size);
		d_output_iter = d_output+(i*FFT_size);

		if (FFT_size < 8192) {
			SSFFT_GPU_Short_base(d_input_iter, d_output_iter, FFT_exp, FFT_size, 1, nSMs);
		}	
		else {
			SSFFT_GPU_Long_base(d_input_iter, d_output_iter, FFT_exp, FFT_size, 1, nSMs);
		}
	}
