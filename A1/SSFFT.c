...
int GPU_FFT_C2C_Stockham(float2 *h_input, float2 *h_smFFT_output, int FFT_exp, int FFT_size, int nFFTs, int nRuns, int SMs, double *single_ex_time);
...
int main(int argc, char* argv[]) {
	if (argc!=5) {
		...
		printf(" 1) FFT length\n");
		printf(" 2) the number of iterations\n");
		printf(" 3) 0: baseline \\\\ 1: SM gating\n");
		printf(" 4) nFFTs\n");
		printf("For example: FFT.exe 1024 1000 6 \n");
        return(1);
    }
	char * pEnd;
	
	int FFT_exp = strtol(argv[1],&pEnd,10);
	int nRuns    = strtol(argv[2],&pEnd,10);
	int nSMs     = strtol(argv[3],&pEnd,10);
    int nFFTs    = strtol(argv[4],&pEnd,10);

	int FFT_size = (int)1 << FFT_exp;

	int Na = (int)1 << (FFT_exp/2);
	int Nb = (int)1 << (FFT_exp-FFT_exp/2);
	...
	// GPU_FFT_C2C_Stockham(h_input, h_cuFFT_output, FFT_exp, FFT_size, nFFTs, nRuns, 0, &cuFFT_execution_time);
	
	//-----------> custom FFT
	double smFFT_execution_time;
	GPU_FFT_C2C_Stockham(h_input, h_smFFT_output, FFT_exp, FFT_size, nFFTs, nRuns, nSMs, &smFFT_execution_time);

	// for (int i = 0; i < Na; i++) {
	// 	for (int j = 0; j < Nb; j++)
	// 		printf("%.0f ", h_cuFFT_output[i*Nb + j].x);
	// 	printf("\n");
	// }
	// printf("\n");
	// for (int i = 0; i < Na; i++) {
	// 	for (int j = 0; j < Nb; j++) 
	// 		printf("%.0f ", h_smFFT_output[i*Nb + j].x);
	// 	printf("\n");
	// }
